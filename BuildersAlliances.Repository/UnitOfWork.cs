...
namespace BuildersAlliances.Repository
{
    ...
    public class UnityOfWork : IDisposable
    {
        ...
        public UnityOfWork()
        {
            _context = new BuildersAlliancesContext();
        }
        ...
        public void SaveChanges()
        {
            ...
            //                objectId =Convert.ToInt64(entry.CurrentValues[typeInfo.IdName]);
            ...
            //                    parentId = Convert.ToInt64(entry.CurrentValues[typeInfo.ParentIdName]);
            //                break;
            //            case EntityState.Deleted:
            //                objectId = Convert.ToInt64(entry.OriginalValues[typeInfo.IdName]);
            //                if (!string.IsNullOrWhiteSpace(typeInfo.ClientIdName))
            //                    //Value that identifies the object for the user (Name, number, etc)
            ...
            //                if (!string.IsNullOrWhiteSpace(typeInfo.ParentIdName))
            //                    //Id of the parent (to be logged in children objects)
            //                    parentId = Convert.ToInt64(entry.OriginalValues[typeInfo.ParentIdName]);
            //                break;
        ...
        public SqlCommand StoreProcedure(string commandText)
        {
            System.Data.Common.DbConnection connection = this._context.Database.Connection;
        ...
        public List<T> ExecuteProcedure<T>(string ProcedureName,params object[] Parameters)
        {
            
             return this._context.Database.SqlQuery<T>(ProcedureName, Parameters).ToList<T>();
            
        }

        public bool ExecuteCommand(string query)
        {
            this._context.Database.ExecuteSqlCommand(query);
            return true;
        }
